    ...
    <footer className="relative border-t border-white/10 bg-slate-950/95">
      ...
      <div className="section-container flex flex-col gap-6 py-8 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-2 text-sm text-gray-400 rotate-[-0.5deg]">
          ...
          <p className="font-serif text-base text-white">© 2026 Lefevralov Store. All rights reserved.</p>
          <p>
            Street high fashion, limited drops, and items that last beyond one season.
            Personal selection and support for every purchase.
          </p>
        </div>
        <div className="flex flex-col items-start gap-3 text-sm sm:items-end rotate-[0.5deg]">
          <div className="flex gap-4">
            ...
            <Link href={AVITO_URL} target="_blank" rel="noreferrer" className="text-gray-300 transition-colors duration-200 hover:text-brand-gold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 focus-visible:rounded">Avito</Link>
          </div>
          <p className="text-xs text-gray-500">Made with love for minimalism and high fashion aesthetics.</p>
        </div>
      </div>
    </footer>
  );
